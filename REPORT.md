...
3. Add the following strings ([FreeBSD](https://www.freebsd.org/)-based mock logs) with append (`>>`), that contain: YYYY-MM-DD pattern, "ERROR", "FAILED", "CRITICAL" & IPv4 address
...
4. Display a detailed list with:
```console
$ ls -l monitor/raw/
```
and count total lines in the 3 `.log` files with:
```console
